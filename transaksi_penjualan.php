<?php
    ...
    $query_penjualan = "SELECT * 
                        ...
                        ORDER BY no_nota_penjualan ASC";

    $query_produk = "SELECT * FROM produk WHERE tanggal_keluar";

    $result_penjualan = mysqli_query($c, $query_penjualan);
...
<head>
    ...
    <title>Toko Jaya Perkasa - Transaksi Penjualan</title>
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="row flex-nowrap">
            ...
            <div class="col py-5">
                <a href="index.php" style="color: black;">Home</a> -> Transaksi Penjualan
                <br><br>
                <p style="font-size: 35px; font-family: Arial, Helvetica, sans-serif;">Penjualan</p>
                <?php if ($_SESSION["nama"] == "Admin") : ?>
                <a href="transaksi_tambah_penjualan.php"><button>+ Tambah Transaksi Penjualan</button></a>
                <?php endif; ?>
                <table border="1" cellpadding="10" cellspacing="0" style="background-color: white;">
                    ...
                    <tr>
                        <td><?= $i; ?></td>  
                        <td><?= $row_penjualan["no_nota_penjualan"]; ?></td>        
                        <td><?= $row_penjualan["tanggal_penjualan"]; ?></td>
                        <td><?= $row_penjualan["jumlah"]; ?></td>
                        <td><?= "Rp " . number_format($row_penjualan["harga"]); ?></td>
                        <td><?= "Rp " . number_format($row_penjualan["harga"] * $row_penjualan["jumlah"]); ?></td>
                        <td>
                            <a href="transaksi_ubah_penjualan.php?id_penjualan=<?= $row_penjualan["id_penjualan"]; ?>&no_nota_penjualan=<?= $row_penjualan["no_nota_penjualan"]; ?>"><button style="background-color: #C0C0C0;">Ubah</button></a>
                            <a href="transaksi_hapus_penjualan.php?id_penjualan=<?= $row_penjualan["id_penjualan"]; ?>&no_nota_penjualan=<?= $row_penjualan["no_nota_penjualan"]; ?>&jumlah=<?= $row_penjualan["jumlah"]; ?>&nama_produk=<?= $row_penjualan["nama_produk"]; ?>" onclick="return confirm('Apakah anda yakin ingin menghapus transaksi ini?')"><button style="background-color: #FB7272;">Hapus</button></a>
                        </td>
                    </tr>
                    <?php $i++ ?>
                    <?php endwhile; ?>

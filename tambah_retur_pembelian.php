<?php
    ...
    $query = "SELECT no_nota_pembelian, nama_produk 
...
<head>
    ...
    <title>Toko Jaya Perkasa - Tambah Retur Pembelian</title>
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="row flex-nowrap">
            ...
            <div class="col py-5">
                <a href="index.php" style="color: black;">Home</a> -> <a href="retur_pembelian.php" style="color: black;">Retur Pembelian</a> -> Tambah Retur Pembelian
                <br><br>
                <p style="font-size: 35px; font-family: Arial, Helvetica, sans-serif;">Tambah Retur Pembelian</p>
                <form action="" method="post">
                    Pilih produk : 
                    <select name="nama_produk">
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                        <option value="<?= $row["nama_produk"]; ?>"><?= $row["no_nota_pembelian"] . " - " . $row["nama_produk"]; ?></option>
                        <?php endwhile; ?>
                    </select><br><br>
                ...
                <a href="retur_pembelian.php"><button>Kembali</button></a><br><br>
                <?php
                    endif;

                    if (isset($_POST["tambahretur"])) {
                        $query_pembelian = "SELECT id_pembelian 
                                            FROM detil_pembelian d
                                            JOIN pembelian j ON d.pembelian_id = j.id_pembelian
                                            JOIN produk p ON d.produk_id = p.id_produk";
                        ...
                        $data = mysqli_fetch_assoc($result_pembelian);
                        ...
                        $query = "INSERT INTO retur_pembelian VALUES ('','$nama_produk','$jumlah','$tanggal_retur','$catatan_retur','$id_pembelian')";

                        mysqli_query($c, $query);
                        mysqli_affected_rows($c);

                        if (mysqli_affected_rows($c) > 0) {
                            echo "<b>Retur pembelian berhasil ditambahkan</b>";
                        } else {

<?php
    ...
    $query = "SELECT no_nota_penjualan, nama_produk 
...
<head>
    ...
    <title>Toko Jaya Perkasa - Tambah Retur Penjualan</title>
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="row flex-nowrap">
            ...
            <div class="col py-5">
                <a href="gudang.php" style="color: black;">Home</a> -> <a href="retur_penjualan.php" style="color: black;">Retur Penjualan</a> -> Tambah Retur Penjualan
                <br><br>
                <p style="font-size: 35px; font-family: Arial, Helvetica, sans-serif;">Tambah Retur Penjualan</p>
                <form action="" method="post">
                    Pilih produk : 
                    <select name="nama_produk">
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                        <option value="<?= $row["nama_produk"]; ?>"><?= $row["no_nota_penjualan"] . " - " . $row["nama_produk"]; ?></option>
                        <?php endwhile; ?>
                    </select><br><br>
                    Jumlah : <input type="text" name="jumlah" required><br><br>
                    Tanggal retur : <input type="datetime-local" name="tanggal_retur" required><br><br>        
                    Catatan : <input type="text" name="catatan_retur" required><br><br>
                    <button type="submit" name="tambahretur">Tambah</button>       
                </form><br>
                <a href="retur_penjualan.php"><button>Kembali</button></a><br><br>
                <?php
                    endif;

                    if (isset($_POST["tambahretur"])) {
                        $query_penjualan = "SELECT id_penjualan 
                                            FROM detil_penjualan d
                                            JOIN penjualan j ON d.penjualan_id = j.id_penjualan
                                            JOIN produk p ON d.produk_id = p.id_produk";
                        ...
                        $data = mysqli_fetch_assoc($result_penjualan);
                        ...
                        $catatan_retur = $_POST["catatan_retur"];
                                
                        $query = "INSERT INTO retur_penjualan VALUES ('','$nama_produk','$jumlah','$tanggal_retur','$catatan_retur','$id_penjualan')";

                        mysqli_query($c, $query);
                        mysqli_affected_rows($c);

                        if (mysqli_affected_rows($c) > 0) {
                            echo "<b>Retur penjualan berhasil ditambahkan</b>";
                        } else {
